...
<?
        ...
        if(empty($arParams["SHOW_SECTION_BANNER"]) || !empty($arParams["SHOW_SECTION_BANNER"]) && $arParams["SHOW_SECTION_BANNER"] == "Y"){
            if(!empty($arCurSection["ID"])){
				...
				if(!empty($arSectionID)){
                    ...
                    $rsSection = CIBlockSection::GetList(array("DEPTH_LEVEL" => "DESC"), array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "ID" => $arSectionID, "ACTIVE" => "Y", "GLOBAL_ACTIVE" => "Y"), false, array("ID", "NAME", "IBLOCK_ID", "UF_BANNER", "UF_BANNER_MOBILE",  "UF_BANNER_LINK", "UF_BANNER_MOBILE_LINK"));
					while($arSection = $rsSection->GetNext()){
						if(!empty($arSection["UF_BANNER_MOBILE"])){
							$arResult["SECTION_BANNERS"]['SECTION_BANNER_MOBILE']["IMAGE"] = CFile::ResizeImageGet($arSection["UF_BANNER_MOBILE"], array("width" => 600, "height" => 14400000), BX_RESIZE_IMAGE_PROPORTIONAL, true);	
							$arResult["SECTION_BANNERS"]['SECTION_BANNER_MOBILE']["IMAGE"]["LINK"] = $arSection["UF_BANNER_MOBILE_LINK"][$ib];
						}
    ...
    <div id="catalog">
        <?if(!empty($arResult["SECTION_BANNERS"])):?>
             <div id="catalog-section-banners">
				<ul class="slideBox">
					<?foreach ($arResult["SECTION_BANNERS"] as $isc => $arNextBanner):?>	
						<?if(!empty($arNextBanner["IMAGE"]) && $isc !== "SECTION_BANNER_MOBILE"):?>						
							<li>
			...
			<div id="catalog-section-banners_mobile" class="mobile">
				<ul class="slideBox">
					<?if(!empty($arResult["SECTION_BANNERS"]["SECTION_BANNER_MOBILE"])):?>
						<li>
							<a<?if(!empty($arResult["SECTION_BANNERS"]['SECTION_BANNER_MOBILE']["IMAGE"]["LINK"])):?> href="<?=$arNextBanner["LINK"]?>"<?endif;?>>
								<img src="<?print_r($arResult["SECTION_BANNERS"]['SECTION_BANNER_MOBILE']["IMAGE"]["src"])?>" class="lazy" alt="">
							</a>
						</li>
					<?endif;?>
				</ul>
				...
				<script>
					$(function() {
						$("#catalog-section-banners_mobile").dwSlider({
							rightButton: ".catalog-section-banners-btn-right",
							leftButton: ".catalog-section-banners-btn-left",
							delay: 6000,
							speed: 1000
