...
function getLastOffer($arLastFilter, $arProps, $productID, $opCurrency, $arPrices = array()){
	...
	if(!$rsLastOffer->SelectedRowsCount()){
		...
		if($obReturnOffer = $rsLastOffer->GetNextElement()){
			...
			if(empty($productFilelds["DETAIL_PICTURE"]) || empty($productProperties["MORE_PHOTO"]["VALUE"]) || empty($productFilelds["PROPERTIES"]["BONUS"]["VALUE"]) || empty($productFilelds["DETAIL_TEXT"]) || empty($productFilelds["PREVIEW_TEXT"])){
				...
				if($rsProduct = CIBlockElement::GetList(array(), array("ID" => $productID), false, false, $rsProductSelect)->GetNextElement()){
					...
					if(empty($productProperties["MORE_PHOTO"]["VALUE"]) || empty($productFilelds["PROPERTIES"]["BONUS"]["VALUE"])){
						$rsProductProperties = $rsProduct->GetProperties(array("sort" => "asc", "name" => "asc"), array("EMPTY" => "N"));
					}

					//preview text
					if(empty($productFilelds["PREVIEW_TEXT"]) && !empty($rsProductFields["PREVIEW_TEXT"])){
						$productFilelds["PREVIEW_TEXT"] = $rsProductFields["PREVIEW_TEXT"];
						$productFilelds["PREVIEW_TEXT_TYPE"] = $rsProductFields["PREVIEW_TEXT_TYPE"];
					}

					//detail text
					if(empty($productFilelds["DETAIL_TEXT"]) && !empty($rsProductFields["DETAIL_TEXT"])){
						$productFilelds["DETAIL_TEXT"] = $rsProductFields["DETAIL_TEXT"];
						$productFilelds["DETAIL_TEXT_TYPE"] = $rsProductFields["DETAIL_TEXT_TYPE"];
					}
