	...
	var sendFastBack = function(event){
		...
		if($fastBuyFormName.val() != "" && $fastBuyFormTelephone.val() !="" && $personalInfo.prop("checked")){
			...
			$.getJSON(ajaxPath + "?" + $fastBuyForm.serialize()).done(function(jData){
				...
				// отправка в б24
				var price = $("#fastBuyPrice").text().replace(" ", "").replace("руб", "");
				var name = $fastBuyForm.find("#fastBuyFormName").val();
				var phone = $fastBuyForm.find("#fastBuyFormTelephone").val();
				var product = $("#fastBuyName .middle").html();
				var mess = $("#fastBuyFormMessage").val();
				
				
				let url = "https://ruteh.bitrix24.ru/rest/3/amtjssesld2yz4cm/crm.lead.add.json";
				let data = {
					fields:
					{ 
						"SOURCE_ID" : "23",
						"TITLE": "Заявка ProDGU.ru", 
						"NAME": name, 
						"STATUS_ID": "NEW", 
						"OPENED": "Y", 
						"ASSIGNED_BY_ID": 52, 
						"PHONE": [ { "VALUE": phone, "VALUE_TYPE": "WORK" } ] ,
						"COMMENTS": 'Ссылка на товар: <a href="' + document.location.href + '">' + product + "</a>" + "<br><br>Комментарий: " + mess,
						"OPPORTUNITY": price
					},
					params: { 
